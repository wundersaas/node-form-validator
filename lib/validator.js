...
class Validator {
  ...
  parseRules(validationRules) {
    ...
    attrs.forEach((attr) => {
      ...
      if (!Array.isArray(rulesArray)) {
        ...
        rulesArray = (strRules || "").toString().split("|");
      }
  ...
  async parseNestedAttr(attr) {
    ...
    for await (const item of Object.keys(value)) {
   ...
   */
  getParsedMessage({ rule, args = [], attr, value, useDefaultMessage = true }) {
    /**
    ...
    if (this.hasCustomMessages) {
      message = this.customMessages[`${attr}.${rule}`] || this.customMessages[attr] || this.customMessages[rule];
    }
    ...
    if (!message) {
      ...
      message =
        messages[this.lang].$custom[`${attr}.${rule}`] ||
        // message.$custom.rule
   ...
   * @param {*} args
   */
  getExistinParsedMessage({ rule, args = [], attr, value }) {
    /**
     * 1. check for attribute.rule
     * 2. check for rule
     * 3. check for attribute
